...
function person() {
            ...
            $.ajax({
                type: "POST",
                url: "../includes/top_person.php",
                data: "",
                success: function (data) {
                    var json = eval("(" + data + ")");
                    $(".loader-page").css({visibility:"hidden",opacity:"0"});
                    var string = "";                     
                    string += '<span class="next-icon"><span><h1>Usuarios que utilizan RedTIL<h1>';
                    if(json[0].res_c=="success"){
                                string += '<div class="cajaproy">';
                                string += '<h4>Usuario cerca de mi<h4>';
                                string += '<div class="owl-one owl-carousel owl-theme" id="comunidad">';
                                  for (var i = 1; i < json.length; i++) {
                                    if(json[i].por=="comunidad"){
                                    string += '<div class="item">';
                                            string += '<div class="cajaproy1">';
                                            string += '    <img onclick="to_it(\''+json[i].url_perfil+'\');" src="../'+ json[i].imagen +'" alt="">';
                                            string += '    <div class="orden"><p onclick="to_it(\''+json[i].url_perfil+'\');">'+json[i].nombre+' '+json[i].apellido+'</p>';
                                            string += '</div>';
                                            string += '</div>';
                                    string += '</div>';
                                    }
                     ...
                     if(json[0].res_t=="success"){
                      ...
                      var cont_edu = [];
                        for (var i = 1; i < json.length; i++) {
                              ...
                              if(tematica=="t_int"){
                                t_int.push(json[i].id_user);
                              }
                              if(tematica=="pro"){
                                pro.push(json[i].id_user);
                              }
                              if(tematica=="arc"){
                                arc.push(json[i].id_user);
                              }
                              if(tematica=="red"){
                                red.push(json[i].id_user);
                              }
                              if(tematica=="org_t"){
                                org_t.push(json[i].id_user);
                              }
                              if(tematica=="edu_t"){
                                edu_t.push(json[i].id_user);
                              }
                              if(tematica=="block"){
                                block.push(json[i].id_user);
                              }
                              if(tematica=="sft_pu"){
                                sft_pu.push(json[i].id_user);
                              }
                              if(tematica=="innova"){
                                innova.push(json[i].id_user);
                              }
                              if(tematica=="apps"){
                                apps.push(json[i].id_user);
                              }
                              if(tematica=="iot"){
                                iot.push(json[i].id_user);
                              }
                              if(tematica=="robot"){
                                robot.push(json[i].id_user);
                              }
                              if(tematica=="geo"){
                                geo.push(json[i].id_user);
                              }
                              if(tematica=="h_lib"){
                                h_lib.push(json[i].id_user);
                              }
                              if(tematica=="gob_dig"){
                                gob_dig.push(json[i].id_user);
                              }
                              if(tematica=="inter"){
                                inter.push(json[i].id_user);
                              }
                              if(tematica=="datos_a"){
                                datos_a.push(json[i].id_user);
                              }
                              if(tematica=="ai"){
                                ai.push(json[i].id_user);
                              }
                              if(tematica=="tresd"){
                                tresd.push(json[i].id_user);
                              }
                              if(tematica=="cont_edu"){
                                cont_edu.push(json[i].id_user);
                              }
                        }     
                              var minimo=2;
                        ...
                        if(cont_edu.length>minimo){
                          ...
                          for (var i = 0; i < cont_edu.length; i++) {
                            var val = cont_edu[i];
                              var index = json.findIndex(function(item, i){
                                return item.id_user == val
                              });
                                string += '<div class="item">';

                                    string += '<div class="cajaproy1">';
                                    string += '    <img onclick="to_it(\''+json[index].url_perfil+'\');" src="../'+ json[index].imagen +'" alt="">';
                                    string += '    <div class="orden"><p onclick="to_it(\''+json[index].url_perfil+'\');">'+json[index].nombre+' '+json[index].apellido+'</p>';
                                    string += '</div>';
                                    string += '</div>';
                                string += '</div>';
                          }//end-for
                          ...
                          string += '<div class="cajaproy">';
                          string += '<h4>Sistemas de Información Geográficos<h4>';
                    ...
                    if(json[0].res_t=="fail" && json[0].res_c=="fail"){
                      var string = '';
                        string += "<div class='nofound'>";
                        string += "<img src='../assets/img/no-found.png'>";
                        string += "<h1>No hay elementos<h1>";
                        ...
                        success: function (data) {
                            ...
                            var string = '';
                              ...
                              string += '<ul>';
                                  ...
                                  string += '</li>';
                                  string += '<li>';
                                  string += '<span>Tema del libro: '+ json[0].tema_libro +'</span>';
                                  string += '</li>';
                                  string += '<li>';
                              ...
                              for (var i = 1; i<json.length; i++) {
                                if (json[i].t_dato == "autor") {
                                  ...
                                  string += '</ul>';
                                   ...
                                   string += '</div>';
                              string += '</div>';
                              string += '<button type="submit" onclick="lib();" class="btn">Libros</button>';
                              string += '</div>';
                              if(json[1].my_val=="done"){
                                  string += '<span class="num">' + json[1].my + '</span><img src="../assets/img/my.png" class="imgp">';
                              }else{
                                  string += '<span class="num">' + json[1].my + '</span><img src="../assets/img/my0.png" onclick="to_me('+id_user+', this);" class="imgp">';
                              }
                                string += '<div class="addcomenta">';                                
                                  string += '<textarea placeholder="Escribe un comentario.." id="public"></textarea>';                  
                                  string += '<button onclick="enviar_coment('+id_user+');" class="btn boton2">Publicar</button></form>';
                                  string += '</div>'; 
        ...
        function update_qrcode(link, id, array) {
            ...
            Swal.fire({
              ...
              html: "<p style='font-size:.7em;'>Comparte este contenido y dejanos saber si te gustó</p><p>" + array["resume_doc"] + "</p><img src='" + src + "' width='50%'>" +
                  ...
                  '<input type="hidden" name="id_user" name="id_user" value="'+ id +'">'+
                            ...
                            if (json.res == "success") {
                               ...
                               var url = document.URL;
                                      ...
                                      for (var i = 0; i < grande; i++) {
                                        result_g +=  caracteres.charAt(Math.floor(Math.random()* caracterestam));
                                      }
                                      for (var i = 0; i < peque; i++) {
                                        result_p +=  caracteres.charAt(Math.floor(Math.random()* caracterestam));
                                      }
                                      result_g = result_g.replace(/mAg/g, 's3q');
                                      result_g = result_g.replace(/nA/g, '32L');
                                      result_p = result_p.replace(/mAg/g, 's3q');
                                      result_p = result_p.replace(/nA/g, '32L');
                                      enlace = url + "?doc=" + result_g + "mAg" + id_user + "nA" + result_p;
                                      $(".loader-page").css({visibility:"visible",opacity:"1"})
                                      update_qrcode(enlace, id_user, vector);
